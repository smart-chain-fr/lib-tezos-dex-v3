...
- [Entrypoints](#entrypoints)
  ...
  - [CFMM-specific on-chain views](#cfmm-specific-views)
    - [**v_get_constants**](#v_get_constants)
    - [**v_get_position_info**](#v_get_position_info)
- [Errors](#errors)
...
The `v_get_constants` on-chain view allows callers (external smart contract) to dynamicaly retrieve the `constants` information from the storage. It is useful to verify the addresses of token.

### **v_get_position_info**

The `v_get_position_info` on-chain view allows callers (external smart contract) to dynamicaly retrieve a `position_index` information (including liquidity , ownership, tick interval) for a given `position_id`. It is helpful for callers to easily get the liquidity of a position.
