...
let default_storage
  ...
  let minus_max_tick : int = 0n - const_max_tick in
  let minus_impossible_tick : int = 0n - impossible_tick in 
  let min_tick_state =
    ...
    ; sqrt_price = half_bps_pow (minus_max_tick, default_ladder)
    } in
...
*)
let main (_param, store : parameter * storage) : result =
