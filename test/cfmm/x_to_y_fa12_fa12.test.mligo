...
let () = Log.describe("[FA12-FA12] [Cfmm.x_to_y] test suite")
...
let test_crossing_tick =
    ...
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = FA12_helper.approve_success((cfmm1.addr, total_liquidity), tokenX.contr) in
    let () = FA12_helper.approve_success((cfmm1.addr, total_liquidity), tokenY.contr) in
    let () = FA12_helper.approve_success((cfmm2.addr, total_liquidity), tokenX.contr) in
