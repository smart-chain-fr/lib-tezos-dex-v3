...
let () = Log.describe("[FA12-FA12] [Cfmm.set_position] test suite")
...
let test_success =
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, user1, 999500n) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm.addr, 500n) in
...
let test_success_set_position_with_no_liquidity_is_noop =
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, user1, 1000000n) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm.addr, 0n) in
...
let test_success_deposit_withdrawal_position =
    ...
    let () = FA12_helper.approve_success((cfmm.addr, 1000000n), tokenX.contr) in
    let () = FA12_helper.approve_success((cfmm.addr, 1000000n), tokenY.contr) in
    let () = FA12_helper.assert_allowance tokenX.taddr user1 cfmm.addr 1000000n in
