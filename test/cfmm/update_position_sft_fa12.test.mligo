...
let () = Log.describe("[SFT-FA12] [Cfmm.update_position] test suite")
...
let test_success_set_and_update =
    ...
    let () = FA12_helper.assert_user_balance_in_range(tokenY.taddr, user1, 1000000n, 1n) in // TODO compute fees (1000000 - fees)
    FA12_helper.assert_user_balance_in_range(tokenY.taddr, cfmm.addr, 0n, 1n) // TODO compute fees (1000000 - fees)
...
let test_success_adding_twice =
    ...
    let () = FA12_helper.approve_success((cfmm1.addr, 5000000n), tokenY.contr) in
    let () = SFT_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm2.addr; token_id=0n})], tokenX.contr) in
    let () = FA12_helper.approve_success((cfmm2.addr, 5000000n), tokenY.contr) in
