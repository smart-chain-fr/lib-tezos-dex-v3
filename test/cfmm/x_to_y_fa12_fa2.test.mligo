...
let () = Log.describe("[FA12-FA2] [Cfmm.x_to_y] test suite")
...
let test_swapping_100_within_a_single_tick_range =
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm.addr, expected_x_initial_balance_for_10000000 + x_amount_swap) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, swapper, abs(swapperBalanceX - x_amount_swap)) in
    ...
    //let () = FA12_helper.assert_user_balance(tokenX.taddr, feeReceiver, expected_fee) in
    let () = FA12_helper.assert_user_balance_in_range(tokenX.taddr, feeReceiver, expected_fee, 1n) in
...
let test_many_small_swaps_equivalent_to_1_big_swap =
    ...
    let () = Test.set_source swapper in
    let () = FA12_helper.approve_success((cfmm1.addr, swapperBalanceX), tokenX.contr) in
    ...
    let () = Test.set_source swapper in
    let () = FA12_helper.approve_success((cfmm2.addr, swapperBalanceX), tokenX2.contr) in
    ...
    let expected_x_initial_balance_for_10000000 = 487706n in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm1.addr, expected_x_initial_balance_for_10000000) in 
    let () = FA12_helper.assert_user_balance(tokenX.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000)) in 
    let expected_y_initial_balance_for_10000000 = 487706n in
    ...
    let expected_x_initial_balance_for_10000000 = 487706n in
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, expected_x_initial_balance_for_10000000) in 
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000)) in 
    let expected_y_initial_balance_for_10000000 = 487706n in
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm1.addr, expected_x_initial_balance_for_10000000 + x_amount_swap * swap_count) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, swapper, abs(swapperBalanceX - (x_amount_swap * swap_count))) in
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, expected_x_initial_balance_for_10000000 + x_amount_swap_cfmm2) in
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, swapper, abs(swapperBalanceX - x_amount_swap_cfmm2)) in
...
let test_crossing_tick =
    ...
    let () = FA12_helper.approve_success((cfmm1.addr, swapperBalanceX), tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm1.addr; token_id=0n})], tokenY.contr) in
    let () = FA12_helper.approve_success((cfmm2.addr, swapperBalanceX), tokenX.contr) in
...
let test_fee_split = 
    ...
    let feeReceiver2_x = FA12_helper.get_user_balance(tokenX.taddr, feeReceiver2) in
...
let test_push_cur_tick_index_just_below_witness  =
    ...
    let (liquidityProvider, swapper, swapReceiver, _feeReceiver, _) = accounts in
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_fa12(0n, 0tez, liquidityProvider, postion_1_liquidity + postion_2_liquidity + swapperBalanceX) in
    ...
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = FA12_helper.approve_success((cfmm.addr, postion_1_liquidity + postion_2_liquidity), tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm.addr; token_id=0n})], tokenY.contr) in
    let () = Test.set_source swapper in
    let () = FA12_helper.approve_success((cfmm.addr, swapperBalanceX), tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm.addr; token_id=0n})], tokenY.contr) in
