...
let () = Log.describe("[SFT-FA12] [Cfmm.complex_position] test suite")
...
let test_position_initialization = 
    ...
    let process(cpd, swap_dir : create_position_data * bool) : unit =
        ...
        let initialCfmmBalanceX = SFT_helper.get_user_balance(tokenX.taddr, cfmm.addr) in
        let initialCfmmBalanceY = FA12_helper.get_user_balance(tokenY.taddr, cfmm.addr) in
        ...
        let finalCfmmBalanceX = SFT_helper.get_user_balance(tokenX.taddr, cfmm.addr) in
