...
let () = Log.describe("[SFT-FA2] [Cfmm.x_to_y] test suite")
...
let test_swapping_100_within_a_single_tick_range =
    ...
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm.addr, expected_x_initial_balance_for_10000000 + x_amount_swap) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, abs(swapperBalanceX - x_amount_swap)) in
    ...
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, expected_y_received_for_100) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, cfmm.addr, abs(expected_y_initial_balance_for_10000000 - expected_y_received_for_100)) in
    ...
    //let () = SFT_Helper.assert_user_balance(tokenX.taddr, feeReceiver, expected_fee) in
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, feeReceiver, expected_fee, 1n) in
    ExtendedFA2_helper.assert_user_balance(tokenY.taddr, feeReceiver, 0n)
...
let test_many_small_swaps_equivalent_to_1_big_swap =
    ...
    let expected_x_initial_balance_for_10000000 = 487706n in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm1.addr, expected_x_initial_balance_for_10000000) in 
    let () = SFT_helper.assert_user_balance(tokenX.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000)) in 
    let expected_y_initial_balance_for_10000000 = 487706n in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, cfmm1.addr, expected_y_initial_balance_for_10000000) in 
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, liquidityProvider, abs(10000000 - expected_y_initial_balance_for_10000000)) in 
    ...
    let expected_x_initial_balance_for_10000000 = 487706n in
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, expected_x_initial_balance_for_10000000) in 
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000)) in 
    let expected_y_initial_balance_for_10000000 = 487706n in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, cfmm2.addr, expected_y_initial_balance_for_10000000) in 
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, liquidityProvider, abs(10000000 - expected_y_initial_balance_for_10000000)) in 
    ...
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm1.addr, expected_x_initial_balance_for_10000000 + x_amount_swap * swap_count) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, abs(swapperBalanceX - (x_amount_swap * swap_count))) in
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, expected_x_initial_balance_for_10000000 + x_amount_swap_cfmm2) in
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, swapper, abs(swapperBalanceX - x_amount_swap_cfmm2)) in
    ...
    // on every swap.
    //let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, expected_y_received_for_100) in
    let () = ExtendedFA2_helper.assert_user_balance_in_range(tokenY.taddr, swapReceiver, expected_y_received_for_100, swap_count) in
    let () = ExtendedFA2_helper.assert_user_balance_in_range(tokenY.taddr, cfmm1.addr, abs(expected_y_initial_balance_for_10000000 - (expected_y_received_for_100)), swap_count) in
    // FOR CFMM2
    let expected_y_received_for_100 = expectedDy_cfmm2 in // 99n
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, swapReceiver, expected_y_received_for_100) in
    ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, cfmm2.addr, abs(expected_y_initial_balance_for_10000000 - (expected_y_received_for_100)))
    ...
    let rec assert_tick_indexes(acc, current, interval : (Cfmm.tick_index * Cfmm.tick_state) list * Cfmm.tick_index * nat) : (Cfmm.tick_index * Cfmm.tick_state) list =
        if (current.i >= 0) then
            ...
            | Some ts -> 
                ...
                // let () = Test.log(abs(ts.seconds_per_liquidity_outside.x128 - expected_splo)) in
...
let test_fee_split = 
    ...
    let feeReceiver2_x = SFT_helper.get_user_balance(tokenX.taddr, feeReceiver2) in
    let () = assert(feeReceiver2_x <> 0n) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, feeReceiver2, 0n) in
    ()
...
let test_push_cur_tick_index_just_below_witness  =
    ...
    let (liquidityProvider, swapper, swapReceiver, _feeReceiver, _) = accounts in
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, liquidityProvider, postion_1_liquidity + postion_2_liquidity + swapperBalanceX) in
    let tokenY = Bootstrap.boot_token(0n, 0tez, liquidityProvider, postion_1_liquidity + postion_2_liquidity + swapperBalanceY) in
    let cfmm = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    // Transfer 200 (token X and Y) to swapper
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceX}]}], tokenX.contr) in
