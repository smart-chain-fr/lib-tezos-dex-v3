...
let () = Log.describe("[SFT-FA2] [Cfmm.update_position] test suite")
...
let test_success_set_and_update =
    ...
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, user1, 1000000n) in
    ...
    let () = SFT_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
...
let test_success_adding_twice =
    let accounts = Bootstrap.boot_accounts() in
    let (user1, _user2, _, _, _) = accounts in
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, user1, 10000000n) in
    let tokenY = Bootstrap.boot_token(0n, 0tez, user1, 10000000n) in
    ...
    let () = Test.set_source user1 in
    let () = SFT_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm1.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm1.addr; token_id=0n})], tokenY.contr) in
    let () = SFT_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm2.addr; token_id=0n})], tokenX.contr) in
    ...
    let cfmm_1_balance_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm1.addr) in
    let () = ExtendedFA2_helper.assert_user_balance_in_range(tokenY.taddr, cfmm2.addr, cfmm_1_balance_y, 1n) in
    ()
...
let test_withdrawal_overflow = 
    ...
    let () = SFT_helper.transfer_success([{from_=user1; txs=[{to_=user2; token_id=0n; amount=5000000n}]}], tokenX.contr) in
    let () = ExtendedFA2_helper.transfer_success([{from_=user1; txs=[{to_=user2; token_id=0n; amount=5000000n}]}], tokenY.contr) in
    // update_operators
    let () = SFT_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm.addr; token_id=0n})], tokenY.contr) in
    let () = Test.set_source user2 in
    let () = SFT_helper.update_operators_success([Add_operator({owner=user2; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=user2; operator=cfmm.addr; token_id=0n})], tokenY.contr) in
...
let test_LPs_get_fees = 
    ...
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, feeReceiver, fee_sum_x, 1n) in
    let () = ExtendedFA2_helper.assert_user_balance_in_range(tokenY.taddr, feeReceiver, fee_sum_y, 1n) in
    ()
...
let test_fees_are_proportional_to_liquidity = 
    ...
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, liquidityProvider, total_supply_x) in
    let tokenY = Bootstrap.boot_token(0n, 0tez, liquidityProvider, total_supply_y) in
    let cfmm = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    // Transfer some (token X and Y) to swapper
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceX}]}], tokenX.contr) in
    ...
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=liquidityProvider2; token_id=0n; amount=position_2_liquidity}]}], tokenX.contr) in
    let () = ExtendedFA2_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=liquidityProvider2; token_id=0n; amount=position_2_liquidity}]}], tokenY.contr) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, liquidityProvider2, position_2_liquidity) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, liquidityProvider2, position_2_liquidity) in
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm.addr; token_id=0n})], tokenY.contr) in
    ...
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider2; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider2; operator=cfmm.addr; token_id=0n})], tokenY.contr) in
    ...
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, feeReceiver1, fee_sum_x / 4n, 1n) in
    let () = ExtendedFA2_helper.assert_user_balance_in_range(tokenY.taddr, feeReceiver1, fee_sum_y / 4n, 1n) in
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, feeReceiver2, fee_sum_x * 3n / 4n, 1n) in
...
let test_LPs_do_not_receive_past_fees = 
    ...
    // Note: Fees are rounded down when being distributed to LPs, so a margin of error of -1 is acceptable.
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, feeReceiver1, fee_sum_x_before + (fee_sum_x_after / 2n), 1n) in
    let () = ExtendedFA2_helper.assert_user_balance_in_range(tokenY.taddr, feeReceiver1, fee_sum_y_before + (fee_sum_y_after / 2n), 1n) in
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, feeReceiver2, fee_sum_x_after / 2n, 1n) in
...
let test_fees_are_discounted = 
    ...
    let initialBalanceLpX = SFT_helper.get_user_balance(tokenX.taddr, liquidityProvider) in
    let initialBalanceLpY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, liquidityProvider) in
    ...
    // VERIFY BALANCES
    let finalBalanceLpX = SFT_helper.get_user_balance(tokenX.taddr, liquidityProvider) in
    let finalBalanceLpY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, liquidityProvider) in
    let finalBalanceFeeReceiverX = SFT_helper.get_user_balance(tokenX.taddr, feeReceiver) in
    let finalBalanceFeeReceiverY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, feeReceiver) in
...
let test_many_small_liquidations = 
    ...
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapper, swapperBalanceY) in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=liquidityProvider2; token_id=0n; amount=liquidity}]}], tokenX.contr) in
    let () = ExtendedFA2_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=liquidityProvider2; token_id=0n; amount=liquidity}]}], tokenY.contr) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, liquidityProvider2, liquidity) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, liquidityProvider2, liquidity) in
    ...
    // VERIFY BALANCES
    let receiver1_x = SFT_helper.get_user_balance(tokenX.taddr, receiver1) in
    let receiver1_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, receiver1) in
    let receiver2_x = SFT_helper.get_user_balance(tokenX.taddr, receiver2) in
    let receiver2_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, receiver2) in
...
let test_updating_nonexisting_position = 
    ...
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, liquidityProvider, liquidity) in
    let tokenY = Bootstrap.boot_token(0n, 0tez, liquidityProvider, liquidity) in
    let cfmm = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
