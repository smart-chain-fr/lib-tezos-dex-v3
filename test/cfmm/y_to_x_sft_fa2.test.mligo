...
let () = Log.describe("[SFT-FA2] [Cfmm.y_to_x] test suite")
...
let test_swapping_1_within_a_single_tick_range =
    ...
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, liquidityProvider, liquidity + swapperBalanceX) in
    ...
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceX}]}], tokenX.contr) in
    ...
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapper, swapperBalanceY) in
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
    ...
    let () = SFT_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
    ...
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm.addr, expected_x_initial_balance_for_10000000) in 
    let () = SFT_helper.assert_user_balance(tokenX.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000)) in 
    let expected_y_initial_balance_for_10000000 = 487706n in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, cfmm.addr, expected_y_initial_balance_for_10000000) in 
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, liquidityProvider, abs(10000000 - expected_y_initial_balance_for_10000000)) in 

    let s_init = Test.get_storage cfmm.taddr in
    let _initial_swapReceiver_balance_x = SFT_helper.get_user_balance(tokenX.taddr, swapReceiver) in
    let _initial_swapReceiver_balance_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, swapReceiver) in
    let _initial_swapper_balance_x = SFT_helper.get_user_balance(tokenX.taddr, swapper) in
    let _initial_swapper_balance_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, swapper) in

    // SWAP 1 Y_TOKEN 
    let () = Test.set_source swapper in
    let y_amount_swap = 1n in
    let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(y_amount_swap, 0n, swapReceiver) in
    ...
    let fee_dy = Maths.calcSwapFee(feeBps, y_amount_swap) in
    let new_dy  = abs(y_amount_swap - fee_dy) in 
    let expected_sqrt_price = Maths.calcNewPriceY(s_init.sqrt_price, liquidity, new_dy, effectiveProtoFeeBps) in
    let () = assert(s_after.sqrt_price.x80 = expected_sqrt_price) in 

    let () = assert(fee_dy >= 1n) in

    // VERIFY fee growth
    let expected_fee =  Bitwise.shift_left (fee_dy) 128n in 
    let expected_fee_growth = expected_fee / liquidity in
    let () = assert(s_after.fee_growth.y.x128 = expected_fee_growth) in
    
    // VERIFY expected DX
    let expectedDx = Maths.calcReceivedX(s_init.sqrt_price.x80, expected_sqrt_price, liquidity) in
   
    // VERIFY TOKEN TRANSFER
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, cfmm.addr, expected_y_initial_balance_for_10000000 + y_amount_swap) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapper, abs(swapperBalanceY - y_amount_swap)) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    // expected_x = ((liquidity * _280) / sqrt_price_new) - ((liquidity * _280) / sqrt_price_old)
    //            = 12089258196146291747061760000000 / 1208937062624751590757501 - 12089258196146291747061760000000 / 1208925819614629174706176n
    //            = -92.9991351087
    let expected_x_received_for_100 = expectedDx in // 92n in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapReceiver, expected_x_received_for_100) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, 0n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm.addr, abs(expected_x_initial_balance_for_10000000 - expected_x_received_for_100)) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in

    // UPDATE POSITION 0 (for getting fees)
    // COLLECT FEES
    ...
    let expected_fee = Maths.calcSwapFee(feeBps, y_amount_swap) in
    let () = ExtendedFA2_helper.assert_user_balance_in_range(tokenY.taddr, feeReceiver, expected_fee, 1n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in
    ()

let test_swapping_10_within_a_single_tick_range =
    let feeBps = 700n in
    let protoFeeBps = 700n in
    let effectiveProtoFeeBps = if config.y = CTEZ then protoFeeBps else 0n in
    let liquidity = 10_000_000n in 
    ...
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapper, abs(swapperBalanceY - y_amount_swap)) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    let expected_x_received_for_10 = expectedDx in // 8n in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapReceiver, expected_x_received_for_10) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, 0n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm.addr, abs(expected_x_initial_balance_for_10000000 - expected_x_received_for_10)) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in
...
let test_swapping_100_within_a_single_tick_range =
    ...
    // SWAP 100 Y_TOKEN 
    let () = Test.set_source swapper in
    let y_amount_swap = 100n in
    let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(y_amount_swap, 50n, swapReceiver) in
    let () = Cfmm_helper.y_to_x_success(param, 0tez, cfmm.contr) in 
    let s_after = Test.get_storage cfmm.taddr in
...
let test_swapping_10000_within_a_single_tick_range =
    ...
    let swapperBalanceX : nat = 10000n in
    let swapperBalanceY : nat = swapperBalanceX in
    let accounts = Bootstrap.boot_accounts() in
    let (liquidityProvider, swapper, swapReceiver, feeReceiver, _) = accounts in
    // Deploy contracts
    ...
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapper, abs(swapperBalanceY - y_amount_swap)) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    let expected_x_received_for_10000 = expectedDx in // 9291n in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapReceiver, expected_x_received_for_10000) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, 0n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm.addr, abs(expected_x_initial_balance_for_10000000 - expected_x_received_for_10000)) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in
...
let test_many_small_swaps_equivalent_to_1_big_swap =
    ...
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm1.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm1.addr; token_id=0n})], tokenY.contr) in
    let () = Test.set_source swapper in
    let () = SFT_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm1.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm1.addr; token_id=0n})], tokenY.contr) in

    // for CFMM2
    // Deploy contracts CFMM1 & tokenX and token Y
    let tokenX2 = Bootstrap.boot_token_sft(0n, 0tez, liquidityProvider, liquidity + swapperBalanceX) in
    let tokenY2 = Bootstrap.boot_token(0n, 0tez, liquidityProvider, liquidity + swapperBalanceY) in
    let cfmm2 = Bootstrap.boot_cfmm_by_config(config, tokenX2.addr, tokenY2.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    // Transfer 1_000_000 (token X and Y) to swapper
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceX}]}], tokenX2.contr) in
    let () = ExtendedFA2_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceY}]}], tokenY2.contr) in
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, swapper, swapperBalanceX) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, swapper, swapperBalanceY) in
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm2.addr; token_id=0n})], tokenX2.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm2.addr; token_id=0n})], tokenY2.contr) in
    let () = Test.set_source swapper in
    let () = SFT_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm2.addr; token_id=0n})], tokenX2.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm2.addr; token_id=0n})], tokenY2.contr) in
    ...
    let expected_x_initial_balance_for_10000000_cfmm1 = 487706n in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm1.addr, expected_x_initial_balance_for_10000000_cfmm1) in 
    let () = SFT_helper.assert_user_balance(tokenX.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000_cfmm1)) in 
    let expected_y_initial_balance_for_10000000_cfmm1 = 487706n in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, cfmm1.addr, expected_y_initial_balance_for_10000000_cfmm1) in 
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, liquidityProvider, abs(10000000 - expected_y_initial_balance_for_10000000_cfmm1)) in 

    let s_init_cfmm1 = Test.get_storage cfmm1.taddr in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapReceiver, 0n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapper, swapperBalanceY) in
    ...
    let expected_x_initial_balance_for_10000000_cfmm2 = 487706n in
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, expected_x_initial_balance_for_10000000_cfmm2) in 
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000_cfmm2)) in 
    let expected_y_initial_balance_for_10000000_cfmm2 = 487706n in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, cfmm2.addr, expected_y_initial_balance_for_10000000_cfmm2) in 
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, liquidityProvider, abs(10000000 - expected_y_initial_balance_for_10000000_cfmm2)) in 
    let s_init_cfmm2 = Test.get_storage cfmm2.taddr in

    // SWAP 10 times 10 Y_TOKEN on CFMM1
    let () = Test.set_source swapper in
    let swap_count = 10n in
    let y_amount_swap = 10n in
    let apply_x_to_y(amount_swap, tick_spacing, receiver: nat * nat * address) : test_exec_result = 
        let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(amount_swap, tick_spacing, receiver) in
        let r = Cfmm_helper.y_to_x(param, 0tez, cfmm1.contr) in
        let () = Assert.tx_success r in
        r
    in
    let gen_args(_count : nat) : (nat * nat * address) = (y_amount_swap, 1n, swapReceiver) in
    let _rrrr = Utils.repeat(swap_count, apply_x_to_y, gen_args, ([]: test_exec_result list)) in
    let s_after_cfmm1 = Test.get_storage cfmm1.taddr in


    // SWAP 100 X_TOKEN on CFMM2
    let () = Test.set_source swapper in
    let y_amount_swap_cfmm2 = 100n in
    let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(y_amount_swap_cfmm2, 50n, swapReceiver) in
    let () = Cfmm_helper.y_to_x_success(param, 0tez, cfmm2.contr) in 
    ...
    // VERIFY expected DY
    let expectedDx_cfmm1 = Maths.calcReceivedX(s_init_cfmm1.sqrt_price.x80, s_after_cfmm1.sqrt_price.x80, liquidity) in
    let expectedDx_cfmm2 = Maths.calcReceivedX(s_init_cfmm2.sqrt_price.x80, s_after_cfmm2.sqrt_price.x80, liquidity) in
    let () = assert(expectedDx_cfmm1 = expectedDx_cfmm2) in

    // VERIFY TOKEN TRANSFER TOKEN_Y
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, cfmm1.addr, expected_y_initial_balance_for_10000000_cfmm1 + y_amount_swap * swap_count) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapper, abs(swapperBalanceX - (y_amount_swap * swap_count))) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, cfmm2.addr, expected_y_initial_balance_for_10000000_cfmm2 + y_amount_swap_cfmm2) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, swapper, abs(swapperBalanceX - y_amount_swap_cfmm2)) in

    // VERIFY TOKEN TRANSFER TOKEN_X
    // FOR CFMM1
    let expected_x_received_for_100_cfmm1 = expectedDx_cfmm1 in // 90n
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, swapReceiver, expected_x_received_for_100_cfmm1, swap_count) in
    let () = SFT_helper.assert_user_balance_in_range(tokenX.taddr, cfmm1.addr, abs(expected_y_initial_balance_for_10000000_cfmm1 - expected_x_received_for_100_cfmm1), swap_count) in
    // FOR CFMM2
    let expected_x_received_for_100_cfmm2 = expectedDx_cfmm2 in // 99n
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, swapReceiver, expected_x_received_for_100_cfmm2) in
    let () = SFT_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, abs(expected_y_initial_balance_for_10000000_cfmm2 - expected_x_received_for_100_cfmm2)) in

    let cfmm1BalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm1.addr) in
    let cfmm2BalanceY = ExtendedFA2_helper.get_user_balance(tokenY2.taddr, cfmm2.addr) in
...
let test_crossing_tick =
    ...
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, liquidityProvider, total_liquidity + swapperBalanceX) in
    let tokenY = Bootstrap.boot_token(0n, 0tez, liquidityProvider, total_liquidity + swapperBalanceY) in
    let cfmm1 = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    ...
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm1.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm1.addr; token_id=0n})], tokenY.contr) in
    let () = SFT_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm2.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm2.addr; token_id=0n})], tokenY.contr) in

    let () = Test.set_source swapper in
    let () = SFT_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm1.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm1.addr; token_id=0n})], tokenY.contr) in
    let () = SFT_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm2.addr; token_id=0n})], tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=swapper; operator=cfmm2.addr; token_id=0n})], tokenY.contr) in
    ...
    // SET POSITION 10 times 1000000 X_TOKEN on CFMM on intervals [-1000; -800] [-800; -600] ... [800; 1000]
    let () = Test.set_source liquidityProvider in
    let apply_set_position(liq, low_tick, high_tick: nat * int * int) : (int * int) = 
        ...
        if (s_temp_cfmm_2.cur_tick_index.i >= high_tick) then
            (high_tick, (t_set_position_temp_before - (0: timestamp)))
        else
            (high_tick, 0)
    in
    ...
    in
    // let _rrrr = Utils.repeat(nb_set_position, apply_set_position, gen_args, ([]: test_exec_result list)) in
    ...
    let () = Test.log("ticks after set position (cfmm2):", tick_list) in 
    ...
    let cfmm1InitialBalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm1.addr) in
    let cfmm2InitialBalanceX = SFT_helper.get_user_balance(tokenX.taddr, cfmm2.addr) in
    let cfmm2InitialBalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm2.addr) in


    // SWAP 200 Y_TOKEN 
    // Place a small swap to move the tick past 0 and advance the time to fill the
    // buffer with _something_ other than zeros.
    let () = Test.log("swap 200 Y") in
    let () = Test.set_source swapper in
    let y_amount_swap = 200n in
    let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(y_amount_swap, 1n, swapReceiver) in
    let r = Cfmm_helper.y_to_x(param, 0tez, cfmm1.contr) in
    let () = Test.log(r) in
    let () = Test.log("swap 200 Y on CFMM2") in
    let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(y_amount_swap, 1n, swapReceiver) in
    let r = Cfmm_helper.y_to_x(param, 0tez, cfmm2.contr) in
    let () = Test.log(r) in 
    ...
    let () = Test.log("swap 50000 Y on CFMM1") in
    let () = Test.set_source swapper in
    let y_amount_swap = 50000n in
    let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(y_amount_swap, 1n, swapReceiver) in
    let r = Cfmm_helper.y_to_x(param, 0tez, cfmm1.contr) in
    let () = Test.log(r) in 
    ...
    let () = Test.log("swap 50000 Y on CFMM2") in
    let r = Cfmm_helper.y_to_x(param, 0tez, cfmm2.contr) in
    let () = Test.log(r) in 
    let t_swap_5000_after = Tezos.get_now() in 
    ...
    let () = assert(((upperTickIndex - 50) <= s_after_cfmm1.cur_tick_index.i) && (s_after_cfmm1.cur_tick_index.i < upperTickIndex + 0)) in
    ...
    let () = assert(feeGrowthX_cfmm1 = {x128=0n}) in
    ...
    // cfmm balances
    let cfmm1FinalBalanceX = SFT_helper.get_user_balance(tokenX.taddr, cfmm1.addr) in
    let cfmm1FinalBalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm1.addr) in
    let cfmm2FinalBalanceX = SFT_helper.get_user_balance(tokenX.taddr, cfmm2.addr) in
    let cfmm2FinalBalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm2.addr) in
    ...
    let () = assert(cfmm1_diff_y = cfmm2_diff_y) in
    ...
    let () = assert((cfmm1_diff_x + 0n <= cfmm2_diff_x) && (cfmm2_diff_x + 0n < cfmm1_diff_x + 10n + 10n))  in
    ...
    let feeReceiver1BalanceX = SFT_helper.get_user_balance(tokenX.taddr, feeReceiver1) in
    let feeReceiver1BalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, feeReceiver1) in
    let feeReceiver2BalanceX = SFT_helper.get_user_balance(tokenX.taddr, feeReceiver2) in
    let feeReceiver2BalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, feeReceiver2) in
    let () = assert(feeReceiver1BalanceX = 0n) in 
    ...
    let rec assert_tick_indexes(acc, current, interval : (Cfmm.tick_index * Cfmm.tick_state) list * Cfmm.tick_index * nat) : (Cfmm.tick_index * Cfmm.tick_state) list =
        if (current.i >= upperTickIndex) then
            acc
        else
            ...
            | Some ts -> 
                ...
                let next_tick = ts.next in
                let () = Test.log("next tick", next_tick) in
                let nts = match Big_map.find_opt next_tick s_after_cfmm2.ticks with
                | Some ts -> ts
                | None -> Test.failwith("next tick not found")
                in
                ...
                in 
                // retrieve timestamp of the set position (for next tick)
                let _expected_so_next_tick = match Big_map.find_opt next_tick expected_ticks_so_map with 
                | Some v -> v
                | None -> Test.failwith("expected tick so not found")
                in
                let () = Test.log("t_swap_5000_before", t_swap_5000_before - (0: timestamp)) in
    ...
    in
    let rec add_interval_rec(current, interval: int * nat) : int =
        if current <= 0 then
            add_interval_rec(current + interval, interval)
        else
            current 
    in
...
let test_fee_split = 
    ...
    let swapperBalanceX : nat = 1000n in
    let swapperBalanceY : nat = 20000n in
    let accounts = Bootstrap.boot_accounts() in
    ...
    let tokenY = Bootstrap.boot_token(0n, 0tez, liquidityProvider, total_liquidity + swapperBalanceY) in
    let cfmm = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    // Transfer 1000 token X and 20000 token Y to swapper
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceX}]}], tokenX.contr) in
    let () = ExtendedFA2_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceY}]}], tokenY.contr) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    ...
    // SET POSITION (1000000) on [100;200]
    let () = Test.set_source liquidityProvider in
    let param : Cfmm.set_position_param = Cfmm_helper.generate_set_position_param(liquidity, (100, 200)) in
    let () = Cfmm_helper.set_position_success(param, 0tez, cfmm.contr) in
    ...
    let feeReceiver1_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, feeReceiver1) in
    ...
    let feeReceiver2_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, feeReceiver2) in
    let () = assert(feeReceiver2_y <> 0n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, feeReceiver2, 0n) in
    ()


// test_must_exceed_min_dx
let test_must_exceed_min_dx =
    let feeBps = 700n in
    let protoFeeBps = 700n in
    let effectiveProtoFeeBps = if config.y = CTEZ then protoFeeBps else 0n in
    let liquidity = 10_000_000n in 
    ...
    let tokenY = Bootstrap.boot_token(0n, 0tez, liquidityProvider, liquidity + swapperBalanceY) in
    let cfmm = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    // Transfer 1 (token X and Y) to swapper
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceX}]}], tokenX.contr) in
    let () = ExtendedFA2_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceY}]}], tokenY.contr) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    ...
    let () = Test.set_source swapper in
    let y_amount_swap = 1n in
    let param : Cfmm.y_to_x_param = Cfmm_helper.generate_y_to_x_param(y_amount_swap, 1000n, swapper) in
...
let test_swaps_are_noops_when_liquidity_is_zero  =
    ...
    let (liquidityProvider, swapper, swapReceiver, _feeReceiver, _) = accounts in
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_sft(0n, 0tez, liquidityProvider, 300n) in // 100 for set_position + 200 for swap
    let tokenY = Bootstrap.boot_token(0n, 0tez, liquidityProvider, 300n) in
    let cfmm = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, feeBps, effectiveProtoFeeBps, 0tez) in
    // Transfer 200 (token X and Y) to swapper
    let () = Test.set_source liquidityProvider in
    let () = SFT_helper.transfer_success([{from_=liquidityProvider; txs=[{to_=swapper; token_id=0n; amount=swapperBalanceX}]}], tokenX.contr) in
