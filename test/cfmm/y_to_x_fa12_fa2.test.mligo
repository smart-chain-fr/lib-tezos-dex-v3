...
let () = Log.describe("[FA12-FA2] [Cfmm.y_to_x] test suite")
...
let test_swapping_1_within_a_single_tick_range =
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm.addr, expected_x_initial_balance_for_10000000) in 
    let () = FA12_helper.assert_user_balance(tokenX.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000)) in 
    ...
    let _initial_swapReceiver_balance_x = FA12_helper.get_user_balance(tokenX.taddr, swapReceiver) in
    let _initial_swapReceiver_balance_y = ExtendedFA2_helper.get_user_balance(tokenY.taddr, swapReceiver) in
    let _initial_swapper_balance_x = FA12_helper.get_user_balance(tokenX.taddr, swapper) in
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, swapReceiver, expected_x_received_for_100) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, 0n) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm.addr, abs(expected_x_initial_balance_for_10000000 - expected_x_received_for_100)) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in
...
let test_swapping_10_within_a_single_tick_range =
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, swapReceiver, expected_x_received_for_10) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, 0n) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm.addr, abs(expected_x_initial_balance_for_10000000 - expected_x_received_for_10)) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in
...
let test_swapping_10000_within_a_single_tick_range =
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, swapReceiver, expected_x_received_for_10000) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY.taddr, swapReceiver, 0n) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm.addr, abs(expected_x_initial_balance_for_10000000 - expected_x_received_for_10000)) in
    let () = FA12_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in
...
let test_many_small_swaps_equivalent_to_1_big_swap =
    ...
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = FA12_helper.approve_success((cfmm1.addr, liquidity), tokenX.contr) in
    ...
    // for CFMM2
    // Deploy contracts CFMM1 & tokenX and token Y
    let tokenX2 = Bootstrap.boot_token_fa12(0n, 0tez, liquidityProvider, liquidity + swapperBalanceX) in
    ...
    // Transfer 1_000_000 (token X and Y) to swapper
    let () = Test.set_source liquidityProvider in
    let () = FA12_helper.transfer_success((liquidityProvider, (swapper, swapperBalanceX)), tokenX2.contr) in
    ...
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, swapper, swapperBalanceX) in
    let () = ExtendedFA2_helper.assert_user_balance(tokenY2.taddr, swapper, swapperBalanceY) in
    // update_operators
    let () = Test.set_source liquidityProvider in
    let () = FA12_helper.approve_success((cfmm2.addr, liquidity), tokenX2.contr) in
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, cfmm1.addr, expected_x_initial_balance_for_10000000_cfmm1) in 
    let () = FA12_helper.assert_user_balance(tokenX.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000_cfmm1)) in 
    ...
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, expected_x_initial_balance_for_10000000_cfmm2) in 
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, liquidityProvider, abs(10000000 - expected_x_initial_balance_for_10000000_cfmm2)) in 
    ...
    let () = FA12_helper.assert_user_balance_in_range(tokenX.taddr, swapReceiver, expected_x_received_for_100_cfmm1, swap_count) in
    let () = FA12_helper.assert_user_balance_in_range(tokenX.taddr, cfmm1.addr, abs(expected_y_initial_balance_for_10000000_cfmm1 - expected_x_received_for_100_cfmm1), swap_count) in
    // FOR CFMM2
    let expected_x_received_for_100_cfmm2 = expectedDx_cfmm2 in // 99n
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, swapReceiver, expected_x_received_for_100_cfmm2) in
    let () = FA12_helper.assert_user_balance(tokenX2.taddr, cfmm2.addr, abs(expected_y_initial_balance_for_10000000_cfmm2 - expected_x_received_for_100_cfmm2)) in
...
let test_crossing_tick =
    ...
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_fa12(0n, 0tez, liquidityProvider, total_liquidity + swapperBalanceX) in
    ...
    let () = FA12_helper.approve_success((cfmm1.addr, liquidity), tokenX.contr) in
    let () = ExtendedFA2_helper.update_operators_success([Add_operator({owner=liquidityProvider; operator=cfmm1.addr; token_id=0n})], tokenY.contr) in
    let () = FA12_helper.approve_success((cfmm2.addr, liquidity), tokenX.contr) in
    ...
    let cfmm1InitialBalanceX = FA12_helper.get_user_balance(tokenX.taddr, cfmm1.addr) in
    let cfmm1InitialBalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm1.addr) in
    let cfmm2InitialBalanceX = FA12_helper.get_user_balance(tokenX.taddr, cfmm2.addr) in
    ...
    // cfmm balances
    let cfmm1FinalBalanceX = FA12_helper.get_user_balance(tokenX.taddr, cfmm1.addr) in
    let cfmm1FinalBalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, cfmm1.addr) in
    let cfmm2FinalBalanceX = FA12_helper.get_user_balance(tokenX.taddr, cfmm2.addr) in
    ...
    let feeReceiver1BalanceX = FA12_helper.get_user_balance(tokenX.taddr, feeReceiver1) in
    let feeReceiver1BalanceY = ExtendedFA2_helper.get_user_balance(tokenY.taddr, feeReceiver1) in
    let feeReceiver2BalanceX = FA12_helper.get_user_balance(tokenX.taddr, feeReceiver2) in
...
let test_fee_split = 
    ...
    let feeReceiver1_x = FA12_helper.get_user_balance(tokenX.taddr, feeReceiver1) in
    ...
    let () = FA12_helper.assert_user_balance(tokenX.taddr, feeReceiver2, 0n) in
    ()
...
let test_swaps_are_noops_when_liquidity_is_zero  =
    ...
    let (liquidityProvider, swapper, swapReceiver, _feeReceiver, _) = accounts in
    // Deploy contracts
    let tokenX = Bootstrap.boot_token_fa12(0n, 0tez, liquidityProvider, 300n) in // 100 for set_position + 200 for swap
    ...
    let cfmm_balance_x = FA12_helper.get_user_balance(tokenX.taddr, cfmm.addr) in
