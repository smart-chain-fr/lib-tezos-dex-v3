...
let () = Log.describe("[SFT-FA12] [Cfmm.set_position] test suite")
...
let test_success =
    ...
    // let () = SFT_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm.addr; token_id=0n})], tokenY.contr) in
    ...
    let () = SFT_helper.assert_user_balance(tokenX.taddr, user1, 999500n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm.addr, 500n) in
    let () = FA12_helper.assert_user_balance(tokenY.taddr, user1, 999250n) in
    FA12_helper.assert_user_balance(tokenY.taddr, cfmm.addr, 750n)
...
let test_success_set_position_with_no_liquidity_is_noop =
    ...
    let () = SFT_helper.assert_user_balance(tokenX.taddr, user1, 1000000n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, cfmm.addr, 0n) in
    let () = FA12_helper.assert_user_balance(tokenY.taddr, user1, 1000000n) in
    let () = FA12_helper.assert_user_balance(tokenY.taddr, cfmm.addr, 0n) in
...
let test_success_deposit_withdrawal_position =
    ...
    let cfmm = Bootstrap.boot_cfmm_by_config(config, tokenX.addr, tokenY.addr, 1n, 10n, 10n, 0tez) in
    let () = Test.set_source user1 in
    // AUTHORIZE OPERATOR (CFMM on user1 funds)
    let () = SFT_helper.update_operators_success([Add_operator({owner=user1; operator=cfmm.addr; token_id=0n})], tokenX.contr) in
    let () = FA12_helper.approve_success((cfmm.addr, 1000000n), tokenY.contr) in
    let () = FA12_helper.assert_allowance tokenY.taddr user1 cfmm.addr 1000000n in
