...
let () = Log.describe("[SFT-FA12] [Cfmm.y_to_x] test suite")
...
let test_swapping_1_within_a_single_tick_range =
    ...
    let _initial_swapReceiver_balance_y = FA12_helper.get_user_balance(tokenY.taddr, swapReceiver) in
    let _initial_swapper_balance_x = SFT_helper.get_user_balance(tokenX.taddr, swapper) in
    let _initial_swapper_balance_y = FA12_helper.get_user_balance(tokenY.taddr, swapper) in
    ...
    let () = FA12_helper.assert_user_balance(tokenY.taddr, cfmm.addr, expected_y_initial_balance_for_10000000 + y_amount_swap) in
    let () = FA12_helper.assert_user_balance(tokenY.taddr, swapper, abs(swapperBalanceY - y_amount_swap)) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, swapper, swapperBalanceX) in
    ...
    let () = FA12_helper.assert_user_balance_in_range(tokenY.taddr, feeReceiver, expected_fee, 1n) in
    let () = SFT_helper.assert_user_balance(tokenX.taddr, feeReceiver, 0n) in
    ()
...
let test_many_small_swaps_equivalent_to_1_big_swap =
    ...
    let () = FA12_helper.assert_user_balance(tokenY.taddr, cfmm1.addr, expected_y_initial_balance_for_10000000_cfmm1) in 
    let () = FA12_helper.assert_user_balance(tokenY.taddr, liquidityProvider, abs(10000000 - expected_y_initial_balance_for_10000000_cfmm1)) in 
    ...
    let () = FA12_helper.assert_user_balance(tokenY2.taddr, cfmm2.addr, expected_y_initial_balance_for_10000000_cfmm2) in 
    let () = FA12_helper.assert_user_balance(tokenY2.taddr, liquidityProvider, abs(10000000 - expected_y_initial_balance_for_10000000_cfmm2)) in 
    ...
    let () = FA12_helper.assert_user_balance(tokenY.taddr, cfmm1.addr, expected_y_initial_balance_for_10000000_cfmm1 + y_amount_swap * swap_count) in
    let () = FA12_helper.assert_user_balance(tokenY.taddr, swapper, abs(swapperBalanceX - (y_amount_swap * swap_count))) in
    let () = FA12_helper.assert_user_balance(tokenY2.taddr, cfmm2.addr, expected_y_initial_balance_for_10000000_cfmm2 + y_amount_swap_cfmm2) in
    let () = FA12_helper.assert_user_balance(tokenY2.taddr, swapper, abs(swapperBalanceX - y_amount_swap_cfmm2)) in
    ...
    let cfmm1BalanceY = FA12_helper.get_user_balance(tokenY.taddr, cfmm1.addr) in
...
let test_crossing_tick =
    ...
    let () = FA12_helper.approve_success((cfmm2.addr, liquidity), tokenY.contr) in
...
let test_fee_split = 
    ...
    let feeReceiver2_y = FA12_helper.get_user_balance(tokenY.taddr, feeReceiver2) in
