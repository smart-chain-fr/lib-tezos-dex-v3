...
let calcReceivedY(sqrt_price_old, sqrt_price_new, liquidity, protoFees : nat * nat * nat * nat) : nat =
    ...
    // let version2_hs = abs(diff * liquidity * abs(10000n - protoFees) / 10000 / _280) in
    // let () = Test.log(version2_hs) in
    let () = assert(version1 = version2_hs) in
    // let dy : int = (sqrt_price_new / _280 - sqrt_price_old / _280) * liquidity in
    // let dy_out = abs(-dy) in
    // let version2_hs :nat = abs(dy_out * abs(10000n - protoFees) / 10000) in
    version1


//calcNewPriceY
// 2^80 * dy / liquidity + sqrt_price_old
let calcNewPriceY(sqrt_price_old, liquidity, dy, proto_fee_bps : Cfmm.x80n * nat * nat * nat) : nat =
    ...
    //let version2_hs = (_280 * dy)  / liquidity + sqrt_price_old.x80 in
...
let liquidityDeltaToTokensDelta(liquidityDelta, lowerTickIndex, upperTickIndex, currentTickIndex, sqrtPrice, ladder : int * int * int * int * Cfmm.x80n * Cfmm.ladder) : (int * int) =
    let sqrtPriceLower : Cfmm.x80n = Cfmm.half_bps_pow(lowerTickIndex, ladder) in // TODO : replace by sqrtPriceFor
    let sqrtPriceUpper : Cfmm.x80n = Cfmm.half_bps_pow(upperTickIndex, ladder) in // TODO : replace by sqrtPriceFor
    // -- Equation 6.29
    // let () = assert(sqrtPrice.x80 >= sqrtPriceLower.x80) in
    // let () = assert(sqrtPriceUpper.x80 >= sqrtPriceLower.x80) in
    // let () = Test.log("sqrtPriceLower") in
    // let () = Test.log(sqrtPriceLower) in
    // let () = Test.log("sqrtPriceUpper") in
    // let () = Test.log(sqrtPriceUpper) in
    // let () = Test.log("sqrtPrice") in
    // let () = Test.log(sqrtPrice) in
    let delta_y = 
