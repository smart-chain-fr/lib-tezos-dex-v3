...
let base_storage (token_id, beneficiary, beneficiary_amount : nat * address * nat) : Token.extended_storage = {
    ledger=(Big_map.literal[((beneficiary, 0n), beneficiary_amount)] : Token.FA2.Ledger.t);
    ...
    extension = {
        ...
        total_supply = beneficiary_amount;
        next_token_id = 1n;
        probationary_deadline = (0: timestamp);
        is_paused = false;
    }
}
...
let get_user_balance(taddr, owner : taddr * address) =
    ...
    Token.FA2.Ledger.get_for_user s.ledger owner 0n

let assert_user_balance(taddr, owner, expected_balance : taddr * address * nat) =
    let s = Test.get_storage taddr in
    let user_balance = Token.FA2.Ledger.get_for_user s.ledger owner 0n in
    // let () = Test.log(s) in 
